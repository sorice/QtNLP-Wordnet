...
class ParseDataNoun():
    ...
    def __init__( self, target_sqlite_db, data_to_parse):
      ...
      for linea in lineas:
          ...
          if(gloss.find('; "') != -1):
           self.__cursor.execute("INSERT OR REPLACE INTO [data_noun] ([synset_offset], [lex_filenum], [ss_type], [w_cnt], [p_cnt], [sense], [sense_es], [sense_long], [sense_long_es], [gloss], [gloss_es]) VALUES (?,?,?,?,?,?,'not','not','not',?,'not') ", (linea[0:8], linea[9:11], linea[12:13], linea[14:16], w_aux+str(self.__cant_pointers), gloss[0:gloss.find('; "')], gloss[gloss.find('; "')+2:gloss.__len__()]))
          else:
           self.__cursor.execute("INSERT OR REPLACE INTO [data_noun] ([synset_offset], [lex_filenum], [ss_type], [w_cnt], [p_cnt], [sense], [sense_es], [sense_long], [sense_long_es], [gloss], [gloss_es]) VALUES (?,?,?,?,?,?,'not','not','not','not','not') ", (linea[0:8], linea[9:11], linea[12:13], linea[14:16], w_aux+str(self.__cant_pointers), gloss))

          all_pointers = palabra[words+5:palabra.__len__()]
          #print all_words
          while(cant_words > 1):
           #print all_words[0:all_words.find(" ")]+"-"+all_words[all_words.find(" ")+1:all_words.find(" ")+2]
           self.__cursor.execute("INSERT OR REPLACE INTO [data_noun_word_lex_id] ([synset_offset], [word], [word_traslated], [lex_id]) VALUES (?,?,'not',?) ", (linea[0:8], lower(all_words[0:all_words.find(" ")]), all_words[all_words.find(" ")+1:all_words.find(" ")+2]))
          ...
          while(self.__cant_pointers > 0):
            ...
            self.__cursor.execute("INSERT OR REPLACE INTO [data_noun_ptr] ([synset_offset], [pointer_symbol], [synset_offset_p], [pos], [source/target]) VALUES (?,?,?,?,?) ", (linea[0:8], (all_pointers[0:all_pointers.find(" ")]).replace("\\","$"), other[0:8], other[9:10], other[11:other.__len__()]))
...
def main():
    p = ParseDataNoun("wordnet.db3","dict/data.noun")
    return 0

    ...
    def __init__( self, target_sqlite_db, data_to_parse):
      ...
      for linea in lineas:
          ...
          while(cant_words > 1):
           ...
           self.__cursor.execute("INSERT OR REPLACE INTO [data_verb_word_lex_id] ([synset_offset], [word], [word_es], [lex_id]) VALUES (?,?,'not',?) ", (linea[0:8], all_words[0:all_words.find(" ")], all_words[all_words.find(" ")+1:all_words.find(" ")+2]))
          ...
          while(self.__cant_pointers > 0):
            ...
            self.__cursor.execute("INSERT OR REPLACE INTO [data_verb_ptr] ([synset_offset], [pointer_symbol], [synset_offset_p], [pos], [source/target]) VALUES (?,?,?,?,?) ", (linea[0:8], all_pointers[0:all_pointers.find(" ")], other[0:8], other[9:10], other[11:other.__len__()]))
...
def main():
    p = ParseDataVerb("db/wordnet.db3","dict/data.verb")
    return 0
